...
struct TimerEntry
{
	CTimer* ptr;          // raw pointer, for fast Pulse() access -- CCore never deletes it directly
	Sqrat::Object handle;  // strong reference; keeps the Squirrel instance (and CTimer) alive
};
...
class CCore
{
	public:
		...
		~CCore();
		void AddTimer(CTimer* pTimer, Sqrat::Object timerHandle);
		void CleanWorld();
		void DropAllTimers();
		void LoadVM();
	...
	public:
		...
		void CancelTimersForPlayer(int playerId);
