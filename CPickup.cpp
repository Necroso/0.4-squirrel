...
void CPickup::SetSingleUse(bool bSingleUse)
{
	functions->SetPickupOption(this->nPickupId, vcmpPickupOptionSingleUse, bSingleUse ? 1 : 0);
}
...
bool CPickup::GetAuto() { return Boolify(functions->IsPickupAutomatic(this->nPickupId)); }
int CPickup::GetAutoTimer() { return functions->GetPickupAutoTimer(this->nPickupId); }

bool CPickup::GetSingleUse(void)
{
	return Boolify(functions->GetPickupOption(this->nPickupId, vcmpPickupOptionSingleUse));
}
...
void CPickup::Delete()
{
	// NOTE: This instance will be deleted automatically due to the internal core callback!
	functions->DeletePickup(this->nPickupId);
}

int CPickup::GetID() { return this->nPickupId; }

bool CPickup::StreamedToPlayer(CPlayer* player)
{
	if (player != nullptr) {
		return Boolify(functions->IsPickupStreamedForPlayer(this->nPickupId, player->nPlayerId));
	}
	return false;
}
