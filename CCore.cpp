...
CCore::~CCore() {
    ...
    for (unsigned int i = 0; i < MAX_CHECKPOINTS; ++i) {
        if (this->checkpointMap[i] != nullptr) {
            delete this->checkpointMap[i];
            this->checkpointMap[i] = nullptr;
        }
    }
...
void CCore::AddTimer(CTimer* pTimer, Sqrat::Object timerHandle) {
    this->timers.push_back(TimerEntry{
        pTimer,
        std::move(timerHandle)
        });
...
void CCore::CleanWorld() {
    ...
    for (unsigned int i = 0; i < MAX_CHECKPOINTS; ++i) {
        if (checkpointMap[i] != nullptr && checkpointMap[i]->isOurs) {
            checkpointMap[i]->Delete();
            checkpointMap[i] = nullptr;
        }
    }
}
